...
## Envases: la excepción adentro de la excepción (16/09/2026)

Calidad lleva además una planilla de **stock de envases** —bolsas, bolsones y
mallas—, y desde el 16/09 vive acá como sección: `/produccion/envases`,
`lib/produccion/envases/`, tablas `produccion_envases_*`. El diseño está en
...
que todo el resto de este módulo. Su planilla es la del almacén clonada —mismos
...
No hay valor de enum nuevo: usa `tiene_acceso_produccion()`,
`puede_editar_produccion()` y `es_admin_produccion()`, las mismas del parte.
