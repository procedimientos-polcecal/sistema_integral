...
import { nivelRrhhDe } from "./auth";

/**
 * Verifica sesión + acceso al módulo RRHH (cualquier nivel). Devuelve una
...
export async function tiene_acceso_check(supabase: SupabaseClient): Promise<NextResponse | null> {
  ...
  const nivel = await nivelRrhhDe(supabase, user.id);
  if (!nivel) return NextResponse.json({ error: "Sin acceso al módulo RRHH" }, { status: 403 });
  return null;
}
