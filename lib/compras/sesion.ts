 ...
 *
 * Además pide las cuatro cosas en paralelo. El rol, los grants y las dos listas
 * de aprobadores no dependen entre sí, y en serie eran cuatro esperas donde
 * alcanza con una.
 *
...
export const permisosComprasActuales = cache(async (): Promise<PermisosCompras> => {
  ...
  const [{ data: usuario }, { data: grants }, { data: enLaLista }, { data: enLaDeOS }] =
    await Promise.all([
      ...
      // La de las órdenes de servicio es otra lista: aprobar un servicio y
      // aprobar un material no los decide necesariamente la misma persona.
      supabase.from("os_aprobadores").select("usuario_id").eq("usuario_id", user.id).maybeSingle(),
    ]);
  ...
  return {
    ...
    puedeAprobarOS: Boolean(enLaDeOS),
