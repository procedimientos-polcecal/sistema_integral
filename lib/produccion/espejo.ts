...
export async function espejarDia(dia: DiaAEspejar): Promise<ResultadoEspejo> {
  ...
  try {
    ...
    // problema. Las tres pestañas son independientes entre sí —no hay una
    // cuenta que cruce producción con despacho dentro de la escritura—, así
    // que frenar Despacho porque a Producción le faltó una columna sería
    // castigar un dato bueno por uno que no lo es. Y no hay riesgo de dejar
    // la planilla "a medias para siempre": el día queda `sheets_pendiente`
    // igual que si no se hubiera escrito nada, y reintentarlo vuelve a mandar
    // las tres pestañas — la que ya estaba bien se reescribe con el mismo
    // valor, porque `escribirCeldas` sobreescribe cada celda y no la suma, así
    // que no hay más costo que la llamada de más.
