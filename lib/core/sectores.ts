 ...
 *
 * Acá está lo que se puede probar sin la base: cómo se agrupan y quién manda
 * sobre cada sector. Cuándo dos nombres son el mismo lo decide el núcleo, en
 * `lib/core/catalogo.ts`, porque la misma regla la usan la importación de RRHH
 * y esta pantalla y tienen que coincidir.
 */
...
export function yaExisteElNombre(
  ...
  return sectores.find((s) => s.id !== exceptoId && claveDeNombre(s.nombre) === clave) ?? null;
}
