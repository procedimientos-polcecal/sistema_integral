...
export interface FilaOrigenDestino {
  origen: string;
  destino: string;
  toneladas: number;
}
...
/**
 * Toneladas por origen y destino de un mes — a diferencia de
 * `toneladasPorYacimientoDesdePesadas`, no se limita a los cuatro
 * yacimientos reconocidos (D1/D6/C1/C3): cualquier texto de ORIGEN/DESTINO
 * de "Datos" entra tal cual (Pavone, Serjen, los galpones, etc.), porque acá
 * la pregunta es de dónde vino y adónde fue cada pesada, no de qué cantera.
 * Tampoco filtra por fletero, por la misma razón que esa función.
 */
export function toneladasPorOrigenDestino(
  pesadas: { fecha: string; origen: string | null; destino: string | null; toneladas: number }[],
  mes: string
): FilaOrigenDestino[] {
  const totales = new Map<string, number>();
  for (const p of pesadas) {
    if (p.fecha.slice(0, 7) !== mes.slice(0, 7)) continue;
    const origen = p.origen?.trim() || "(sin origen)";
    const destino = p.destino?.trim() || "(sin destino)";
  ...
  return [...totales.entries()]
    .map(([clave, toneladas]) => {
      ...
      return { origen, destino, toneladas };
    })
    .sort((a, b) => b.toneladas - a.toneladas);
}

export interface FilaDetallePorFecha {
  fecha: string;
  tipo: string;
  origen: string;
  destino: string;
...
/**
 * El detalle día a día de un mes: por fecha, tipo de material, origen y
 * destino. Junta en una fila las pesadas de un mismo día que comparten los
 * tres —si no, un día con 40 camiones daría 40 filas iguales salvo por la
 * hora, que acá no importa—. `tipo` queda como código (`dolomita_d1`), igual
 * que el resto de las funciones de este archivo: la etiqueta la resuelve
 * quien pinta la pantalla con `tipoDeAcarreo` de `acarreo.ts`.
 */
export function detallePorFecha(
  pesadas: { fecha: string; tipo: string | null; origen: string | null; destino: string | null; toneladas: number }[],
  mes: string
): FilaDetallePorFecha[] {
  const totales = new Map<string, number>();
  for (const p of pesadas) {
    ...
    const origen = p.origen?.trim() || "(sin origen)";
    const destino = p.destino?.trim() || "(sin destino)";
    const clave = `${p.fecha}|${tipo}|${origen}|${destino}`;
    totales.set(clave, (totales.get(clave) ?? 0) + p.toneladas);
  }
  return [...totales.entries()]
    .map(([clave, toneladas]) => {
      const [fecha, tipo, origen, destino] = clave.split("|");
      return { fecha, tipo, origen, destino, toneladas };
    })
    .sort((a, b) => (a.fecha === b.fecha ? a.origen.localeCompare(b.origen) : a.fecha.localeCompare(b.fecha)));
}
