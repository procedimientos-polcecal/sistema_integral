...
describe("totalesDeConsumos", () => {
  it("suma USD y ARS y desglosa por tipo", () => {
    const consumos = [
      { tipo: "detonador", cantidad: 48.5, precio_usd: 4.24 },
      { tipo: "otros_insumos", cantidad: 30, precio_usd: 5.34 },
      { tipo: "voladura", cantidad: 1, precio_usd: 17.46 },
    ];
    const r = totalesDeConsumos(consumos, 1515);
    expect(r.totalUsd).toBeCloseTo(383.3, 1);
    expect(r.totalArs).toBeCloseTo(383.3 * 1515, 0);
    expect(r.porTipo.detonador.usd).toBeCloseTo(205.64, 2);
    expect(r.porTipo.voladura.ars).toBeCloseTo(17.46 * 1515, 0);
  });
