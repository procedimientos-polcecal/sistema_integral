  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <a href="/api/remises/export?scope=hist" className="text-sm text-[var(--primary)] hover:underline">Exportar historial</a>
      </div>
    ...
    <section>
      ...
      <div className="card p-5">
        {!plantillas ? (
          ...
          <p className="text-sm text-slate-500">Todavía no hay plantillas guardadas.</p>
        ) : (
            ...
            {plantillas.map((p) => (
              <div key={p.id} className="border border-slate-200 rounded-md px-3 py-2">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="text-xs text-slate-400">{p.tipo === "ida" ? "Ida" : "Vuelta"} · {p.remises_turnos?.nombre} · {empleadosUnicos(p)} empleados</span>
                  <div className="ml-auto flex gap-3">
                    <button onClick={() => setAplicando({ p, fecha: hoy() })} className="text-[var(--primary)] underline text-xs">Aplicar</button>
                ...
                {aplicando && aplicando.p.id === p.id && (
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <button onClick={() => setAplicando(null)} className="text-xs text-slate-500 hover:underline">Cancelar</button>
                  </div>
                )}
              </div>
            ))}
    ...
    <section>
      ...
      <div className="space-y-3">
        {!grupos ? (
          ...
          <div className="card p-5 text-sm text-slate-500">Todavía no se generaron rutas.</div>
        ) : (
          grupos.map((g) => {
            ...
            return (
              <div key={clave} className="card p-4">
                <div className="flex items-center gap-3 flex-wrap">
                  <span className="font-medium text-sm text-slate-800">{fmt(g.fecha)}</span>
                  <span className="text-xs text-slate-400">{g.tipo === "ida" ? "Ida" : "Vuelta"} · {g.turnoNombre}</span>
                  <span className="text-xs text-slate-400">{g.hojas.length} remis</span>
                  <div className="ml-auto flex gap-3">
                    <button onClick={() => setReutilizando({ g, fecha: hoy() })} className="text-[var(--primary)] underline text-xs">Reutilizar</button>
                    <button onClick={() => setGuardandoPlantilla({ g, nombre: `${g.turnoNombre} ${g.tipo === "ida" ? "IDA" : "VUELTA"}` })} className="text-slate-700 underline text-xs">Guardar como plantilla</button>
                  </div>
                </div>
                ...
                {reutilizando && reutilizando.g === g && (
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <button onClick={() => setReutilizando(null)} className="text-xs text-slate-500 hover:underline">Cancelar</button>
                  </div>
                )}
                {guardandoPlantilla && guardandoPlantilla.g === g && (
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <button onClick={() => setGuardandoPlantilla(null)} className="text-xs text-slate-500 hover:underline">Cancelar</button>
                  </div>
                )}
              </div>
            );
