    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-2 mb-3">
          <button onClick={() => setJustificada(true)} className={`flex-1 py-1.5 rounded-md text-sm ${justificada ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600"}`}>Justificada</button>
          <button onClick={() => setJustificada(false)} className={`flex-1 py-1.5 rounded-md text-sm ${!justificada ? "bg-red-600 text-white" : "bg-slate-100 text-slate-600"}`}>Injustificada</button>
        </div>
        {justificada && (
          <div className="mb-3">
        ...
        <div className="mb-4">
          <label className="block text-xs text-slate-500 mb-1">Observaciones {justificada && tipo === "OTRA" ? "(obligatorio: aclarar el motivo)" : ""}</label>
  ...
  return (
    <div>
      ...
      <div className="card p-5 mb-6">
        <h2 className="font-medium text-slate-700 mb-3">Faltas sin clasificar ({faltas.length})</h2>
    ...
    <div>
      <div className="flex gap-4 items-end mb-6 card p-4">
        <div>
          ...
          <input type="date" value={dia} onChange={(e) => setDia(e.target.value)} className="border border-slate-300 rounded-md px-2 py-1.5 text-sm" />
        </div>
      </div>
              ...
              {(roster?.empleados ?? []).map((e) => (
                <tr key={e.employeeId} className="border-b last:border-0">
                  ...
                  <td className="py-2 text-right whitespace-nowrap">
                    <button onClick={() => setEmpleadoEnEdicion(e)} className="text-slate-500 hover:text-blue-600 text-xs underline mr-3">Corregir</button>
                    {e.ausente && e.justificada === null && (
                      <button onClick={() => setSeleccion({ employeeId: e.employeeId, fecha: dia, nombre: e.nombre })} className="text-slate-700 underline text-sm">
                        Clasificar
                      </button>
                    )}
                  </td>
