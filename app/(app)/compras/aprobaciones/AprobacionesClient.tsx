    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500">
          {pendientes.length === 0
            ? "No hay requerimientos esperando decisión."
            : `${pendientes.length} requerimiento${pendientes.length === 1 ? "" : "s"} esperando decisión`}
        </p>
      </div>
      ...
      {pendientes.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-300 px-6 py-14 text-center text-sm text-slate-400">
          Todo al día. No queda nada pendiente de aprobación.
        </div>
      ) : (
        <>
          ...
          {!puedeAprobar && (
            <p className="text-sm text-slate-500">
              Podés consultar la cola, pero no aprobar. Aprobar requiere nivel de administrador
              del módulo Compras.
            </p>
          )}
        </>
