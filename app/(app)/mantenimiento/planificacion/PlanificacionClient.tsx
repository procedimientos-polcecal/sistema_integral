    ...
    <div className="md:p-6 max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">Programá el trabajo del día y generá las órdenes para imprimir</p>
        </div>
        {canEdit && (
          <button onClick={() => setShowNew(true)}
            className="flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700 transition-colors shrink-0">
      ...
      {plans.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-200 py-16 text-center">
          ...
          <p className="text-slate-400 text-sm">Todavía no hay planes. Creá el primero.</p>
        </div>
      ) : (
          ...
          {plans.map((p) => {
            ...
            return (
              ...
              <Link href={`/mantenimiento/planificacion/${p.id}`} className="flex items-center gap-4 flex-1 min-w-0">
                <div className={`shrink-0 w-14 rounded-xl text-center py-1.5 ${isToday ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-700"}`}>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-xs text-slate-400">{itemCount > 0 ? `${itemCount} OT` : "—"}</span>
      ...
      {showNew && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl space-y-4">
            <h2 className="text-base font-bold text-slate-900">Nuevo plan de trabajo</h2>
            <form onSubmit={create} className="space-y-4">
              <div className="space-y-1">
                <label className="block text-xs font-medium text-slate-600">Fecha <span className="text-red-500">*</span></label>
                <input type="date" value={fecha} onChange={e => setFecha(e.target.value)}
                  className="input w-full" required />
              </div>
              <div className="space-y-1">
                <label className="block text-xs font-medium text-slate-600">Título (opcional)</label>
