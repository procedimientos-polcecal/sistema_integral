    ...
    <div className="md:p-6 max-w-4xl mx-auto space-y-4">
      ...
      {tab === "pendientes" && (
        <div className="space-y-2">
          {schedules.length === 0 && <p className="text-sm text-slate-400 py-8 text-center">Sin mantenimientos activos.</p>}
          {schedules.map((s: any) => {
            const overdue = s.next_date && s.next_date < today;
            const soon = s.next_date && s.next_date >= today &&
              s.next_date <= dentroDeUnaSemana;
            return (
              <div key={s.id} className={`rounded-xl border bg-white p-4 flex items-start justify-between gap-4 ${overdue ? "border-red-200 bg-red-50" : "border-slate-200"}`}>
                <div className="space-y-0.5 min-w-0">
                  <p className="text-sm font-medium text-slate-900">{s.equipos?.code} — {s.equipos?.name}</p>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <div className={`text-xs font-semibold ${overdue ? "text-red-700" : soon ? "text-yellow-700" : "text-slate-600"}`}>
      ...
      {tab === "recientes" && (
        <div className="card divide-y divide-slate-100 overflow-hidden">
          {executions.length === 0 && <p className="text-sm text-slate-400 py-8 text-center">Sin ejecuciones registradas.</p>}
          {executions.map((e: any) => (
            <div key={e.id} className="px-4 py-3 flex items-start justify-between gap-4">
              <div className="min-w-0">
                <p className="text-sm font-medium text-slate-900">{e.schedule?.equipos?.code} — {e.schedule?.equipos?.name}</p>
                <p className="text-xs text-slate-500">
                  {e.schedule?.maintenance_type} · {nombreCompleto(e.executor)}
                  {e.duration_hours && ` · ${e.duration_hours} h`}
                </p>
                {e.observations && <p className="text-xs text-slate-400 mt-0.5 line-clamp-1">{e.observations}</p>}
              ...
              <div className="shrink-0 text-right space-y-0.5">
                ...
                <p className="text-xs text-slate-400">{new Date(e.executed_at).toLocaleDateString("es-AR")}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 space-y-4 shadow-xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-base font-bold text-slate-900">Registrar ejecución</h2>
            ...
            <div className="space-y-3">
              ...
              {checklist?.items?.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-medium text-slate-600 uppercase tracking-wide">{checklist.name}</p>
                  {checklist.items.map((item: any) => (
                    <div key={item.id} className="rounded-lg border border-slate-200 p-3 space-y-1">
                      ...
                      {item.type === "photo" && (
                        <p className="text-xs text-slate-400">Usá la sección de fotos abajo.</p>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-2">
                <p className="text-xs font-medium text-slate-600">Fotos (máx. 5)</p>
                ...
                {photos.length < 5 && (
                  <label className="flex items-center gap-2 cursor-pointer rounded-lg border border-dashed border-slate-300 px-3 py-2 text-sm text-slate-500 hover:border-blue-400 hover:text-blue-600 transition-colors w-fit">
